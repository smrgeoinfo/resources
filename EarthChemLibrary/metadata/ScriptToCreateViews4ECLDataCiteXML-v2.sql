...
-- SMR 2017-12-06  
-- rename iedakeywords to keyword_ieda
-- replace author_list1_* field names with keyword_* field name
-- rename eclkeywod to keyword_ecl
-- replace person with person1 in table names
-- update vw_xml_creator_lu to use new person1 and person_external_identifer1
 ...
 SELECT s.submission_id,
    xmlagg(
		XMLELEMENT(NAME subject, 
			XMLATTRIBUTES(kt.keyword_type_label AS "subjectScheme"), ik.keyword_label) 
		ORDER BY kt.keyword_type_label
	) AS subject_xml
   FROM submissions s join submission_keyword_map skm on s.submission_id = skm.submission_id
     join keyword_ieda ik on skm.keyword_id = ik.keyword_id
     join keyword_type kt on ik.keyword_type_id = kt.keyword_type_id
...
CREATE  VIEW public.vw_xml_ecl_subject AS 
   ...
   FROM submissions s join submission_keyword_map skm on s.submission_id = skm.submission_id
     join keyword_ecl ek on skm.keyword_id = ek.keyword_id
...
-- smr 2017-12-06  this view should no longer be necessary because of updates to author list and 
--  person external identifer tables.  Comment out
...
-- 2017-12-06 update to use new person and person external identifer tables
 ...
 SELECT al.submission_id AS s_id,
    xmlagg(XMLELEMENT(NAME creator, 
		XMLELEMENT(NAME "creatorName", 
			btrim(concat(p.last_name, ',', p.first_name, ' ', p.middle_initial))), 
		XMLELEMENT(NAME "givenName", 
			btrim(concat(p.first_name, ' ', p.middle_initial))), 
		XMLELEMENT(NAME "familyName", btrim(p.last_name)),
 
		( SELECT theids.xmlnameid
           FROM ( SELECT p1.person_num,
                    xmlagg(
						XMLELEMENT(NAME "nameIdentifier", XMLATTRIBUTES(pei.external_identifier_scheme AS "nameIdentifierScheme"), pei.external_identifier_value)) AS xmlnameid
                   FROM person_external_identifer1 pei
                     LEFT JOIN person1 p1 on pei.person_num=p1.person_num
                  GROUP BY p1.person_num) theids
          WHERE theids.person_num = p.person_num),
		  
		CASE
            WHEN p.email IS NOT NULL AND p.email <> ''::text THEN 
				XMLELEMENT(NAME "affiliation",  right ( p.email, (length(p.email) - position('@' in p.email)) ))
            ELSE NULL::xml
        END
	    ) ORDER BY al.author_order) AS xmlcreator
   FROM author_list1 al
     LEFT JOIN person1 p ON p.person_num = al.person_num
   GROUP BY al.submission_id;

-- ALTER TABLE public.vw_xml_creator_lu
  -- OWNER TO postgres;
COMMENT ON VIEW public.vw_xml_creator_lu
  IS 'Joins author_list1, person, person_external_identifier, bridge_person_num_distinctname and vw_lookup_ids2use to generate DataCite xml ''creator'' elements based on the author list for the submission.  Returns submission_id and xmlCreator';
...
--2017-10-17 adapt to use new keyword_ecl and iedakeyword tables SMR
CREATE OR REPLACE VIEW public."vw_datacite_xml_generator" AS 
 SELECT s.submission_id,
	...
	XMLELEMENT(NAME language,
		...
		XMLELEMENT(NAME "alternateIdentifier", XMLATTRIBUTES('URL'::text AS "alternateIdentifierType"), concat('http://www.earthchem.org/library/browse/view?id='::text, s.submission_id))), 
		
		-- remove submission ID as alternate identifer (its still in the URL...)
		--XMLELEMENT(NAME "alternateIdentifier", XMLATTRIBUTES('IEDA submission_ID'::text AS "alternateIdentifierType"), concat('urn:grl:submission:'::text, s.submission_id::text)),
		
		XMLELEMENT(NAME "relatedIdentifiers",
        CASE
...
CREATE OR REPLACE VIEW public."vw_datacite_xml_generator_withStatus" AS 
 SELECT s.submission_id,
	...
	XMLELEMENT(NAME language,
		...
		XMLELEMENT(NAME "alternateIdentifier", XMLATTRIBUTES('URL'::text AS "alternateIdentifierType"), concat('http://www.earthchem.org/library/browse/view?id='::text, s.submission_id))), XMLELEMENT(NAME "relatedIdentifiers",
