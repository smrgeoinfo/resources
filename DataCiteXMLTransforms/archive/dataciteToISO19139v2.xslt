...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable name="metadataContactOrg" 
		select="string('EarthChem ECL | Interdisciplinary Earth Data Alliance | Lamont Doherty Earth Observatory | Columbia University')"/>
<!--	<xsl:variable name="metaContactEmail"	select="string('web@usap-dc.org')"/>-->
	<xsl:variable name="metaContactEmail"	select="string('info@iedadata.org')"/>
<!--	<xsl:variable name="metaContactLogo" select="string('http://api.usap-dc.org/images/logo.png')"/>-->
	...
	<xsl:variable name="metaMaintenanceNote"
		select="string('This metadata record was generated by an xslt transformation from DataCite metadata records exported from 
	...
	<xsl:template match="/">
		<gmd:MD_Metadata xmlns:gmd="http://www.isotc211.org/2005/gmd"
			...
			<!-- the fileIdentifier and a datestamp of the conversion 
					fits for harvesting. The fileIdentifier identifies the 
				metadata record, not the described resource which is identified by a DOI.
			If there is a DOI, build URN using that (take out / and :; otherwise generate
			a checksum value based on concatenating the datacite Required fields-->
			<xsl:variable name="theFileIdentifier">
				<xsl:value-of select="//*[local-name() = 'title']"/>
				<xsl:value-of select="//*[local-name() = 'publisher']"/>
			...
			<gmd:fileIdentifier>
				<gco:CharacterString> 
					<xsl:choose>
						<xsl:when test="exists($datacite-identifier) and string-length($datacite-identifier)>0">
							<xsl:value-of
								select="concat($fileIdentifierPrefix, string('metadataabout:'),normalize-space(translate($datacite-identifier,'/:','--')))"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of
								select="concat($fileIdentifierPrefix, string('metadata:'),normalize-space(string(foo:checksum($theFileIdentifier))))"/>
						</xsl:otherwise>
					</xsl:choose>
				</gco:CharacterString>
			</gmd:fileIdentifier>
 ...
 -->
		...
		<xsl:for-each select="//*[local-name() = 'subject' and @subjectScheme != '']">
			<gmd:descriptiveKeywords>
				<gmd:MD_Keywords>
					...
					<xsl:choose>
						<xsl:when test="(lower-case(@subjectScheme)='place') or contains(lower-case(@subjectScheme),'geograph') or contains(lower-case(@subjectScheme),'location')">
							<gmd:type>
								<gmd:MD_KeywordTypeCode codeList="http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#MD_KeywordTypeCode" codeListValue="place" codeSpace="ISOTC211/19115">place</gmd:MD_KeywordTypeCode>
							</gmd:type>
						</xsl:when>
						<xsl:when test="(lower-case(@subjectScheme)='datatype') or contains(lower-case(@subjectScheme),'theme')">
							<gmd:type>
	...
	<xsl:template name="relatedResources">
		...
		<xsl:for-each select="$relres/*">
			<xsl:choose>
				<xsl:when test="not(contains(lower-case(normalize-space(string(.))),'missing')) and	not(contains(lower-case(normalize-space(string(.))),'unknown'))">
			<gmd:aggregationInfo>
